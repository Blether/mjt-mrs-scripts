...
if ! test -f $datadir/${scanid}/${series}.fid/RAW
	then
	${HOME}/.lcmodel/varian/varian2raw $datadir/${scanid}/${series}.fid/fid $datadir/${scanid}/${series}.fid
	echo "varian bin2raw done"
fi

# RAW file for non-water suppressed if appropriate
ecc="0"
if test -f $datadir/${scanid}/${wseries}.fid/fid
	then
	ecc="1"
	h2file="$datadir/${scanid}/${wseries}.fid/RAW"
	if ! test -f $h2file
		then
		${HOME}/.lcmodel/varian/varian2raw $datadir/${scanid}/${wseries}.fid/fid $datadir/${scanid}/${wseries}.fid
	fi
fi

# --------------
# generate the lcm .CONTROL file
controlfile="$outdir/te${te}/${scanid}_${series}.control"
echo " \$LCMODL" > $controlfile
echo " OWNER='FMRIB Centre, University of Oxford'" >> $controlfile
echo " KEY=217638264" >> $controlfile
echo " TITLE= '${scanid} press ${series}'" >> $controlfile
echo " FILBAS='${HOME}/basisfiles/press_te${te}_3t_01b.basis'" >> $controlfile
echo " FILPS='$outdir/te${te}/${scanid}_${series}.ps'" >> $controlfile
echo " FILTAB='$outdir/te${te}/${scanid}_${series}.table'" >> $controlfile
...
echo " SDDEGP=6" >> $controlfile
echo " FILRAW= '$datadir/${scanid}/${series}.fid/RAW'" >> $controlfile
